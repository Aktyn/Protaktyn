...
if show_gui():
    ...
    class GUI:
        def __init__(self, on_init: Callable[[GUIEvents], None]):
            ...
            self.__window.setStyleSheet(
                f"background-color: {GUIConsts.BACKGROUND_COLOR}; color: {GUIConsts.TEXT_COLOR}")
    ...
    from src.modules.scoreboard.view import MockScoreboardViewEvents
    from src.modules.robot.view import MockRobotViewEvents


    class GUIEvents(MockRobotViewEvents, MockScoreboardViewEvents):
        close = MockPyQtSignal()
        show_confirmation_info = MockPyQtSignal()
        close_confirmation_info = MockPyQtSignal()
        set_info_text = MockPyQtSignal()
